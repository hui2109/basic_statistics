...
if __name__ == '__main__':
    ...
    # NCRT治疗前AE
    data_1 = {
        'cross-validation training set': (0.5050, 0.5715),
        'cross-validation validation set': (0.6318, 0.4825),
        'testing set': (0.5016, 0.6377)
    }
    species = ['mean' + sample, 'Z-score']
    ...
    # NCRT治疗前RF
    data_2 = {
        'cross-validation training set': (1.0000, 1.0000),
        'cross-validation validation set': (0.5562, 0.6050),
        'testing set': (0.6861, 0.7480)
    }
    species = ['mean', 'Z-score' + sample]
    draw_grouped_histogram(ax[0, 1], species, data_2, 'RF model before NCRT')
    add_fig_num(ax[0, 1], 'B')

    # NCRT治疗后AE
    # 懒得找了，写了个工具
    source_3_1 = open_specific_path('ceus_post', 'Mean', 'PCC', 'KW_8', 'AE')
    ...
    # NCRT治疗后AE
    source_4_1 = open_specific_path('ceus_post', 'Mean', 'PCC', 'KW_4', 'RF')
    source_4_2 = open_specific_path('ceus_post', 'Zscore', 'PCC', 'KW_4', 'RF')
    data_4 = {
        'cross-validation training set': (
    ...
    plt.savefig('./saved_figs/Figure 1.jpg', bbox_inches='tight')
    plt.show()
